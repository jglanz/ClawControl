...
export class WhisperStreamManager extends EventEmitter {
	...
	start():boolean {
		...
		this.proc.stderr?.on('data', (data:Buffer) => {
			...
			if (!text) return
			if (text.includes('model') || text.includes('CUDA') || text.includes('GPU') || text.includes('error') || text.includes('fail')) {
				log.info(`[whisper:stderr] ${text}`)
			} else {
				log.debug(`[whisper:stderr] ${text}`)
			}
		})
